...
#get rid of workers who on average rated grayscale images as greater than 1.5
grayscale_filtered_data <- d %>%
  group_by(workerNum) %>%
  mutate(gray_mean = mean(score[d$image_type == 'grayscale']) ) %>%
  filter(gray_mean < 1.5)
...
ggplot(fit$model, aes_string(x = names(fit$model)[2], y = names(fit$model)[1])) + 
  ...
  labs(title = paste("Adj R2 = ",signif(summary(fit)$adj.r.squared, 5),
                     ...
                     " P =",signif(summary(fit)$coef[2,4], 5)), x = "predicted colorfulness ratings", y = "observed colorfulness ratings")
}
